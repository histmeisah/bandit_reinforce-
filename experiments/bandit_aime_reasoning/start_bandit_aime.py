"""
Launch script for Bandit-REINFORCE++ training on AIME with 19 prompts.

This script:
1. Loads AIME configuration via Hydra
2. Initializes BanditAgenticPipeline with full prompt selection
3. Runs training with integrated NeuralUCB prompt selection

Usage:
    python experiments/bandit_aime_reasoning/start_bandit_aime.py

    # With config overrides:
    python experiments/bandit_aime_reasoning/start_bandit_aime.py \
        rollout_batch_size=256 \
        max_steps=2000 \
        bandit_config.exploration_param=2.5
"""
...
def main():
    parser = argparse.ArgumentParser(description="Bandit-AIME Training with 19 Prompts")
    parser.add_argument(
        "--config_path",
        help="The path of the main configuration file",
        default="../../experiments/bandit_aime_reasoning"
    )
    parser.add_argument(
        "--config_name",
    ...
    # Initialize Hydra
    initialize(config_path=args.config_path, job_name="bandit_aime")
    cfg = compose(config_name=args.config_name)

    logger.info("=" * 80)
    logger.info("Bandit-REINFORCE++ AIME Training with 19 Prompts")
    logger.info("=" * 80)
    ...
    if bandit_components is not None:
        ...
        bandit_actor = BanditActor.remote(
            ...
            exploration_param=bandit_cfg.get("exploration_param", 2.0),
            neural_ucb_kwargs={
                ...
                "update_freq": bandit_cfg.get("update_freq", 5),
